...
conexion = create_engine('mysql+pymysql://root:administrador@localhost/facultad')
# Consulta SQL.
Consulte = '''
...
df = pd.read_sql(Consulte, con=conexion)
...
df.to_sql(nombre_tabla_nueva, con=conexion, index=False, if_exists='replace') #El if_exists = replace, es la mejor opcion, pero consume  mas recursos.
# Cerrar la conexión.
conexion.dispose()